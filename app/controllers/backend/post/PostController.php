    ...
    public function index() {
        ...
        if (\Input::get('s_name')) {
            $post = \DB::table('post')
                    ...
                    ->select('post.id', 'post.name', 'categories.title as groupname', 'post.disabled', 'post.frontend', 'post.created_user', 'post.created_at', 'post.updated_at')
                    ->orderBy('post.id', 'desc');
            if (\Input::has('s_name')) {
                $post = $post->orWhere('post.name', 'LIKE', '%' . trim(\Input::get('s_name')) . '%');
            }
            $rs = $post->paginate(20);
        } else {
            $rs = \DB::table('post')
