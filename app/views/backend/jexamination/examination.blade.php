    ...
    <div class="col-sm-12">
        <section class="panel">
            <div class="panel-body">
                ...
                <a href="javascript:;" id="btnAdd" class="btn btn-mini btn-primary"><i class="icon-plus"></i> {{\Lang::get('jexamination.add')}}</a>
                @endif
                @if (User::find(Auth::user()->id)->can('edit'))
                <a href="#" class="btn btn-mini btn-success" id="btnEdit"><i class="icon-edit"></i> {{\Lang::get('jexamination.edit')}}</a>
                ...
                <button type="button" class="btn btn-danger" id='btnDelete'><i class="icon-trash"></i> {{\Lang::get('jexamination.delete')}}</button>
                @endif
                <button type="button" class="btn btn-info" id='btnGenLink'><i class="icon-share-alt"></i> Gen Link</button>
            </div>
        </section>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('#btnEdit').click(function() {
        ...
        {
            window.open(base_url + index_page + 'backend/jexamination/examination/edit/' + $(".checkboxes:checked").val(), '_self');
        } else {
    ...
    $('#btnView').click(function() {
        if ($(".checkboxes:checked").val())
        {
            window.open(base_url + index_page + 'backend/jexamination/examination/view/' + $(".checkboxes:checked").val(), '_newtab');
        } else {
            alert('กรุณาเลือกรายการ !');
        }
    });
    ...
    $('#btnDelete').click(function() {
        var data = {
            url: 'backend/jexamination/examination/delete',
