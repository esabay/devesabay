    ...
    <div class="col-sm-12">
        <section class="panel">
            <div class="panel-body">
                ...
                <a href="{{\URL::to('backend/jcareer/jobdescription/add')}}" class="btn btn-mini btn-primary"><i class="icon-plus"></i> {{\Lang::get('jcareer.add')}}</a>
                @endif
        ...
        <section class="panel">
            ...
            <table class="table table-striped border-top" id="sample_1">
                <thead>
                    <tr>
                        <th></th>
                        <th>{{\Lang::get('jcareer.title')}}</th>
                        <th class="hidden-phone">{{\Lang::get('jcareer.department')}}</th>
                ...
                <tbody>
                    ...
                    <tr class="odd gradeX">
                        ...
                        <td>{{$item->title}}</td>
                        <td>{{\Categories::getRootName($item->department_id)}}</td>
                        <td>
                            @if($item->disabled == 0)
...
<script type="text/javascript">
    $('#btnEdit').click(function() {
        ...
        {
            window.open(base_url  + 'backend/jcareer/jobdescription/edit/' + $(".checkboxes:checked").val(), '_self');
        } else {
    ...
    $('#btnView').click(function() {
        if ($(".checkboxes:checked").val())
        {
            window.open(base_url  + 'backend/jcareer/jobdescription/view/' + $(".checkboxes:checked").val(), '_newtab');
        } else {
            alert('กรุณาเลือกรายการ !');
        }
    });
    ...
    $('#btnDelete').click(function() {
        var data = {
            url: 'backend/jcareer/jobdescription/delete',
            title: 'Delete Examination',
            redirect: 'backend/jcareer/jobdescription',
            table_id: '#sample_1'
        };
        deleteData(data);
    });
